...
#session = boto3.Session(region_name='us-east-1',
 #                           aws_access_key_id=os.environ['AKIAIXI5AO2DLXJLHA6Q'],
 #                           aws_secret_access_key=os.environ['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'])

dynamodb = boto3.resource('dynamodb', region_name='us-east-1')

table = dynamodb.Table('icare-mobilehub-1941226567-icaretable')
...
while True:
	...
	temperature = get_sensor_data()
 	...
 	serialNo = str(loopCount)
	message = {}
	message = table.put_item(
	   Item={
		'serialNo':serialNo,
		'temp': str(bpm),
        	'PulseSensorValues': bpm,
        	'ObjectTemperature': int(temperature),
        	'pulse':bpm
        	}
	)
